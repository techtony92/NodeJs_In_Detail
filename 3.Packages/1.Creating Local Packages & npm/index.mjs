 ...
 * * Node, Npm 
 * 
 * * Node comes installed with a package manager called `npm`.
 * * Npm is a package manager that allows us to search, install, build modules written by other community members.
 * * To keep track of packages we install ,we use the `npm init` command to create a `package.json` file. 
*/
